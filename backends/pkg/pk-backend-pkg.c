...
static PkBackendPkgPrivate *priv;
...
pk_backend_initialize (GKeyFile *conf, PkBackend *backend)
{
	...
	/* create private area */
	priv = g_new0 (PkBackendPkgPrivate, 1);
//	priv->use_trusted = TRUE;
	...
	if (err != EPKG_OK)
		g_error("%s", "couldn't init pkg");

	// read configs? libpkg might do that already.
}
...
{
	/* FIXME, this list needs to be revised later on */
...
pk_backend_cancel_timeout (gpointer data)
{
	...
	PkBackendPkgJobData *job_data = pk_backend_job_get_user_data (job);
 ...
 */
/* FIXME impliment me, FreeBSD base upgrade here? On the other hand 9 -> 10 might be more fitting
void
...
pk_backend_get_repo_list (PkBackend *backend, PkBackendJob *job, PkBitfield filters)
{
	...
	{
		const gchar *name = pkg_repo_name(r);
		const gboolean enabled = pkg_repo_enabled(r);
		pk_backend_job_repo_detail (job, name, name, enabled);
	}
	pk_backend_job_finished (job);
}
 ...
 */
/* TODO not supported due to pkg not supporting it
void
...
{
	// pkg doesn't handle multiple jobs at the same time.
	return FALSE;
}
...
{
	/* write of the GSOC project Matt Windsor <mattbw@FreeBSD.org> */
	return "Koop Mast <kwm@rainbow-runner.nl>";
}
